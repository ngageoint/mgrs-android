...
public class MGRS {
     ...
     */
    private static final String latBands = "CDEFGHJKLMNPQRSTUVWXX"; // X is repeated for 80-84°N
    ...
    public MGRS(Integer zone, Character band, Character e100k, Character n100k, Long easting, Long northing) {
        this.zone = zone;
        this.band = band;
        this.e100k = e100k;
        this.n100k = n100k;
    ...
    public static MGRS from(LatLng latLng) {
        ...
        Character e100k = e100kLetters[(utm.getZoneNumber() - 1) % 3].charAt(column - 1); // col-1 since 1*100e3 -> A (index 0), 2*100e3 -> B (index 1), etc.

        // rows in even zones are A-V, in odd zones are F-E
        int row = (int) Math.floor(utm.getNorthing() / 100000) % 20;
        Character n100k = n100kLetters[(utm.getZoneNumber() - 1) % 2].charAt(row);
    ...
    public static MGRS parse(String mgrs) throws ParseException {
        ...
        String[] numericLocations = {numericLocation.substring(0, precision),numericLocation.substring(precision)};

        // parse easting & northing
        double multiplier = Math.pow(10.0, 5 - precision);
        Long easting = new Double(Double.parseDouble(numericLocations[0]) * multiplier).longValue();
        Long northing = new Double(Double.parseDouble(numericLocations[1]) * multiplier).longValue();
    ...
    public UTM utm() {
        ...
        double col = e100kLetters[(zone - 1) % 3].indexOf(e100k) + 1; // index+1 since A (index 0) -> 1*100e3, B (index 1) -> 2*100e3, etc.
        ...
        double nBand = Math.floor(UTM.from(new LatLng(latBand, 0)).getNorthing() / 100000) * 100000;
    ...
    public static String get100KId(double easting, double northing, Integer zoneNumber) {
        ...
        Character e100k = e100kLetters[(zoneNumber - 1) % 3].charAt(column - 1); // col-1 since 1*100e3 -> A (index 0), 2*100e3 -> B (index 1), etc.
